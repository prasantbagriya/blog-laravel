...
import { ArrowBigUp, ArrowBigDown, MessageSquare, Share2, MoreHorizontal, AlertTriangle, ShieldAlert, Link as LinkIcon, Flag, Trash, Bookmark, Share, Link2, X } from 'lucide-react';
        ...
        >
            ...
            <div className="p-4 sm:p-5 pt-4 flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 text-[12px] text-slate-500 dark:text-zinc-400 mb-2.5 font-medium flex-wrap">
                    <Link href={`/community/${post.community?.name || post.community}`} className="font-extrabold text-slate-900 dark:text-white hover:underline flex items-center gap-1.5">
                        <div className="w-5 h-5 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-[10px] text-blue-600 dark:text-blue-400 shrink-0 border border-blue-200 dark:border-blue-800">
                            {(post.community?.name || post.community).charAt(0).toUpperCase()}
                        </div>
                ...
                <div className="flex gap-2 -ml-1.5 mt-2 flex-nowrap whitespace-nowrap overflow-x-auto pb-1" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                    <Link href={`/r/${post.community?.name || post.community}/comments/${post.id}/${post.slug}`} className="shrink-0 whitespace-nowrap flex items-center gap-2 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/40 px-3 py-2 rounded-xl transition-colors font-bold text-[13px] group">
                    ...
                    <PostDropdown post={post} auth={auth} onReport={handleReport} />
                </div>
                
                {/* Fallback internal ReportModal if not provided by parent */}
                <ReportModal 
