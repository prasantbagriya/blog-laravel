...
import { ArrowBigUp, ArrowBigDown, MessageSquare, Share, Bookmark, MoreHorizontal, Search, Plus, Flag, Link as LinkIcon, Trash } from 'lucide-react';
...
export default function Show({ auth, community, posts }) {
    return (
            ...
            <header className="sticky top-0 z-50 bg-white border-b border-[#EDEFF1]">
                <div className="w-full px-4 sm:px-6 h-14 flex items-center justify-between">
                    ...
                    <div className="hidden sm:block flex-1 max-w-2xl mx-8 relative">
                        ...
                        <input 
                            type="text" 
                            placeholder={`Search in r/${community.name}`} 
                    ...
                    <div className="flex items-start pb-4">
                        ...
                        <div className="ml-4 mt-3 flex-1 flex flex-wrap items-start justify-between gap-4">
                            ...
                            <div className="flex gap-2 mt-1">
                                <button className="px-8 py-1.5 bg-[#4F46E5] hover:bg-[#4338CA] text-white font-bold rounded-full transition-colors text-[14px]">
                                    Join
                                </button>
                    ...
                    {posts.map((post) => (
                        <div key={post.id} className="bg-white border border-[#EDEFF1] hover:border-[#878A8C] rounded-md flex cursor-pointer transition-colors">
                            ...
                            <div className="p-2 pt-2.5 flex-1 min-w-0">
                                ...
                                <Link href={`/r/${community.name}/comments/${post.id}/${post.slug}`} className="block">
                                    <h3 className="text-[18px] font-semibold text-[#1C1C1C] mb-2 pr-4">{post.title}</h3>
                                </Link>
                                ...
                                <div className="flex gap-1 -ml-1 mt-1">
                                    <Link href={`/r/${community.name}/comments/${post.id}/${post.slug}`} className="flex items-center gap-1.5 px-2 py-1.5 hover:bg-[#F6F7F8] rounded-sm transition-colors cursor-pointer text-[#878A8C]">
                                    ...
                                    <button 
                                        onClick={(e) => {
                                            e.preventDefault();
                                            navigator.clipboard.writeText(`${window.location.origin}/r/${community.name}/comments/${post.id}`);
                                            alert('Link copied to clipboard!');
                                        }}
