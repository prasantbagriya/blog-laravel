      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div className="p-6 rounded-md bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h1 className="text-xl font-bold text-zinc-900 dark:text-white">AI Moderation & Fraud Inspection Queue</h1>
              <p className="text-xs text-zinc-500 dark:text-zinc-400">Real-time inspection queue for flagged reviews and suspicious AI risk markers.</p>
            </div>
          </div>
        ...
        {moderationQueue.length === 0 ? (
          <div className="p-16 text-center bg-zinc-50 dark:bg-zinc-900 rounded-md border border-zinc-200 dark:border-zinc-800">
            ...
            <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">No pending flagged reviews require human inspection.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-3">
              <span className="text-xs font-mono font-bold uppercase tracking-widest text-zinc-400 block px-1">Flagged Items ({moderationQueue.length})</span>
                ...
                <div
                  ...
                  className={`p-4 rounded-sm border cursor-pointer transition ${
                    activeReview?.id === rev.id
                      ? 'bg-blue-100 dark:bg-blue-600/20 border-blue-600 text-zinc-900 dark:text-white'
                      : 'bg-zinc-50 dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 hover:border-blue-600/30'
                  }`}
                >
                  <div className="flex items-center justify-between text-xs font-bold mb-1">
                    <span className="text-zinc-900 dark:text-white">{rev.reviewerName}</span>
                    <span className="px-2 py-0.5 rounded-2xs bg-rose-500/10 text-rose-600 font-mono text-[10px]">
                      Fraud Score: {rev.aiFraudScore}/100
                    </span>
                  </div>
                  <h4 className="text-xs font-bold text-zinc-900 dark:text-zinc-100 truncate">"{rev.title}"</h4>
                  <p className="text-[11px] text-zinc-500 dark:text-zinc-400 line-clamp-1 mt-0.5">{rev.description}</p>
                </div>
              ))}
            </div>
            ...
            {activeReview && (
              <div className="lg:col-span-2 p-6 rounded-md bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 space-y-6">
                ...
                <div className="space-y-3 text-xs">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-zinc-900 dark:text-white text-sm">"{activeReview.title}"</span>
                    <div className="flex items-center text-amber-500">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <Star key={s} className={`w-3.5 h-3.5 ${s <= activeReview.rating ? 'fill-amber-500 text-amber-500' : 'text-zinc-200 dark:text-zinc-700'}`} />
                      ))}
                    </div>
                  </div>

                  <p className="p-3 rounded-sm bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-white leading-relaxed border border-zinc-200 dark:border-zinc-800">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div className="p-3 rounded-sm bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-800">
                    <span className="text-zinc-400 block text-[10px] font-mono font-bold uppercase">Device Fingerprint</span>
                    <code className="text-zinc-900 dark:text-white font-mono">{activeReview.deviceFingerprint || 'fp_mac_881'}</code>
                  </div>

                  <div className="p-3 rounded-sm bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-800">
                    <span className="text-zinc-400 block text-[10px] font-mono font-bold uppercase">IP Location</span>
                    <span className="text-zinc-900 dark:text-white font-medium">{activeReview.ipLocation || 'Verified ISP'}</span>
                  </div>
                </div>

                {/* Moderator Decision Notes */}
                <div className="space-y-3 pt-2">
                  <label className="block text-xs font-mono font-bold uppercase tracking-widest text-zinc-400">Moderator Audit Note</label>
                  <textarea
