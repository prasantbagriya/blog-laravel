            ...
            <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 max-w-7xl mx-auto w-full px-[15px] pb-8" style={{ paddingTop: '100px' }}>
                <main className="w-full lg:w-[70%]">
                    ...
                    <header className="mb-8">
                        {post.category && (
                            <Link href={`${window.BASE_PATH}/category/${post.category.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`} className="text-blue-600 font-bold uppercase tracking-widest text-sm mb-3 block hover:underline">
                                {post.category}
                            </Link>
                        )}
                        <h1 className="text-4xl md:text-5xl font-bold mb-4 text-slate-900 dark:text-white">{post.title}</h1>
                        ...
                        <div className="mt-12 pt-8 border-t border-slate-200 dark:border-zinc-800">
                            ...
                            <ul className="space-y-2 text-sm text-slate-600 dark:text-zinc-400">
                                {post.sources.map((source, idx) => (
                                    <li key={idx}>
                                        <span className="mr-2 text-slate-500 dark:text-zinc-500">[{idx + 1}]</span>
                                        {source.url ? (
                                            <a href={source.url} target="_blank" rel="nofollow noreferrer" className="hover:text-blue-600 dark:hover:text-blue-400 hover:underline">
                    ...
                    {post.tags && post.tags.length > 0 && (
                        <div className="mt-8 flex flex-wrap gap-2">
                            {post.tags.map((tag, idx) => (
                                <Link key={idx} href={`${window.BASE_PATH}/category/${tag.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`} className="px-4 py-2 bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-zinc-300 rounded-lg text-sm font-medium hover:bg-slate-200 dark:hover:bg-zinc-700 dark:hover:text-zinc-100 transition-colors">
                                    #{tag}
                                </Link>
                            ))}
                        </div>
                    ...
                    {/* About the Author */}
                    <div className="mt-12 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-8 shadow-sm">
                        <div className="flex flex-col sm:flex-row gap-6 items-start">
                            ...
                            <div className="flex-1">
                                ...
                                {post.authorBio && (
                                    <p className="text-slate-600 dark:text-zinc-400 leading-relaxed text-sm mb-4">
                                        {post.authorBio}
                                    </p>
                                )}

                                <div className="flex flex-col sm:flex-row gap-4 sm:gap-8 mt-4 pt-4 border-t border-slate-100 dark:border-zinc-800/50">
                                    {post.authorExperienceYears > 0 && (
                                        <div>
                                            <span className="block text-xs font-bold text-slate-600 dark:text-zinc-500 uppercase tracking-wider mb-1">Experience</span>
                                            <span className="text-sm font-semibold text-slate-700 dark:text-zinc-300">{post.authorExperienceYears}+ Years</span>
                                        </div>
                                    )}

                                    {post.authorAwards && post.authorAwards.length > 0 && (
                                        <div>
                                            <span className="block text-xs font-bold text-slate-600 dark:text-zinc-500 uppercase tracking-wider mb-1">Awards</span>
                                            <div className="flex items-center gap-1 text-sm font-semibold text-amber-600 dark:text-amber-500">
                                    ...
                                    {post.authorAlumniOf && post.authorAlumniOf.length > 0 && (
                                        <div>
                                            <span className="block text-xs font-bold text-slate-600 dark:text-zinc-500 uppercase tracking-wider mb-1">Alumni</span>
                                            <span className="text-sm font-semibold text-slate-700 dark:text-zinc-300">{post.authorAlumniOf[0].name}</span>
                                        </div>
                                    )}
                                </div>
